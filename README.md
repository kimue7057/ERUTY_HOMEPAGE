...
| `/resources` | `src/app/pages/ResourcesPage.tsx` | 부분 완료 · 구조와 탭은 구현, 현재 데이터는 모두 draft |
...
- **완료 — 프로젝트:** Featured 1개를 포함한 공개 프로젝트 11개, 카테고리 필터, 상세 Drawer, 한·영문 콘셉트 이미지 표시가 구현되어 있습니다.
- **완료 — Hitpick:** 서비스 소개, 실행 구조, 기술 엔진, 시장 신호, 파트너 역할, 블록체인 신뢰 레이어와 인터랙션이 구현되어 있습니다.
- **완료 — 이룸터:** AX 전환 과정, 교육, 솔루션, Before/After 및 반응형 Sticky Process가 구현되어 있습니다.
- **완료 — 기술:** 글로벌 마켓 인텔리전스, AX 실행·자동화 오케스트레이션, 블록체인 신뢰·권리·정산 레이어와 React 기반 UI 목업이 구현되어 있습니다.
- **부분 완료 — 리소스:** 6개 카테고리와 sticky 탭은 구현되어 있으나 모든 항목이 `draft`라 공개 카드가 없습니다.
...
- **프로젝트 이미지:** 11개 모두 `public/images/projects/`의 1600×1000 WebP 콘셉트 목업입니다. 한국어는 `콘셉트 이미지`, 영문은 `CONCEPT IMAGE`로 표시됩니다.
- **서비스 데이터:** Hitpick과 이룸터의 소개·과정·기술·적용 내용이 각 서비스 페이지에 반영되어 있습니다.
- **리소스 데이터:** 데이터 항목은 존재하지만 모두 `draft`이며 공개 목록에는 노출되지 않습니다.
...
- 공개 가능한 리소스 실데이터 추가와 `published` 전환 검토
...
- 리소스 데이터는 모두 draft이므로 현재 화면에는 카테고리별 Empty State가 표시됩니다.
